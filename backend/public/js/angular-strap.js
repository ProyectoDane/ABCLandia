  ...
  this.$get = [
    ...
    function ($window, $rootScope, $compile, $q, $templateCache, $http, $animate, $timeout, $sce, dimensions) {
      ...
      function ModalFactory(config) {
        ...
        $modal.hide = function (cleanBody) {
          scope.$emit(options.prefixEvent + '.hide.before', $modal);
          $animate.leave(modalElement, function () {
            scope.$emit(options.prefixEvent + '.hide', $modal);
            
            if (cleanBody) bodyElement.removeClass(options.prefixClass + '-open');
            if (cleanBody && options.animation) {
        ...
        $modal.$onKeyUp = function (evt) {
          evt.which === 27 && scope.$hide();
        };
        // Private methods
        function hideOnBackdropClick(evt) {
          if (evt.target !== evt.currentTarget)
            return;
          options.backdrop === 'static' ? $modal.focus() : scope.$hide();
        }
        return $modal;
      }
      // Helper functions
